...
public class AlunoController {
    ...
    public AlunoController(AlunoRepository alunoRepository){ this.alunoRepository = alunoRepository; }

    @GetMapping
    public ResponseEntity<List<Aluno>> alunoRetrieveAll(){ return ResponseEntity.status(HttpStatus.OK).body(alunoRepository.findAll()); }

    @GetMapping(path = "/{id}")
    public ResponseEntity<Aluno> retrieveColaboradorById(@PathVariable long id){

        Optional<Aluno> o =  alunoRepository.findById(id);
        if(o.isEmpty()) throw new AlunoNotFoundException("ID " +id+ " não encontrado");
